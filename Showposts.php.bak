...
$query = "SELECT * FROM magkl572";

if($limit){
	$query = "SELECT * FROM magkl572 ORDER BY entry_date DESC LIMIT 5";
}

if($search){
	$query = "SELECT * FROM magkl572 WHERE (entry_author LIKE '%$search%' OR entry_text LIKE '%$search%' OR entry_heading LIKE '%$search%') ORDER BY entry_date";
